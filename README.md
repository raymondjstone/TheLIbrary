...
| Activity | `/activity` | Read-only audit trail of consequential file actions (archive / delete / auto-archive) — what moved, when, and what triggered it (`user` vs a job). Backed by `GET /api/activity` and the `ActivityLog` table |
