...
| Activity | `/activity` | Read-only audit trail of consequential actions — files archived / deleted / auto-archived, **manual books added** (`manual-add`), and **manual books promoted** to their real OpenLibrary work (`promote-manual`, by the author-refresh or promote-manual-books job) — what changed, when, and what triggered it (`user` vs a job). Each row carries the book title, author, series and year. Backed by `GET /api/activity` and the `ActivityLog` table |
