...
| Manual Books | `/manual-books` | Every manually-added book (works not on OpenLibrary), **grouped by author** with a filter per column (title, author, year, series, owned), inline edit and delete. Unowned rows carry the **same external book-search links** (Z-Library / NZB indexers) an OpenLibrary book gets elsewhere — since a manual book isn't on OL, those searches are the main way to go find it. The daily [promote-manual-books job](#promote-manual-books-job) links each one to its real OpenLibrary work once OL lists it |
