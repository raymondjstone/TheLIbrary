...
internal static class TestEpub
{
    ...
    // Builds an EPUB whose manifest/spine hrefs are URL-encoded while the actual
    // zip entries keep their literal (decoded) names — reproducing the real-world
    // case where a naive case-sensitive GetEntry misses every spine document and
    // a readable book is mis-scored as 0 pages.
    public static byte[] BuildWithEncodedHrefs(params (string Href, string Html)[] docs)
    {
        ...
        using (var zip = new ZipArchive(ms, ZipArchiveMode.Create, leaveOpen: true))
        {
            ...
            {
                var id = $"c{i}";
                var encoded = Uri.EscapeDataString(docs[i].Href); // e.g. "ch 1.xhtml" -> "ch%201.xhtml"
                manifest.Append($"<item id=\"{id}\" href=\"{encoded}\" media-type=\"application/xhtml+xml\"/>");
                spine.Append($"<itemref idref=\"{id}\"/>");
                Write(zip, $"OEBPS/{docs[i].Href}", docs[i].Html); // entry keeps the literal name
            }
    ...
    // Builds an EPUB whose OPF carries explicit dc:title / dc:creator metadata —
    // for tests of the embedded-metadata-first guessing.
    public static byte[] BuildWithMetadata(string title, string author, params (string Href, string Html)[] docs)
    {
        using var ms = new MemoryStream();
        using (var zip = new ZipArchive(ms, ZipArchiveMode.Create, leaveOpen: true))
        {
